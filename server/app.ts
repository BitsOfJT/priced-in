...
function apiCachePath() {
  return process.env.VERCEL ? join('/tmp', 'priced-in-cache-data.json') : join(process.cwd(), 'server', 'cache-data.json')
}

const cachePath = apiCachePath()
...
function saveCache(key: string, value: unknown, ttlMs: number) {
...
async function fetchJson(url: string) { const response = await fetch(url, { signal: AbortSignal.timeout(12000), headers: { 'User-Agent': 'Priced-In comparison app' } }); if (!response.ok) throw new Error(`Provider returned ${response.status}`); return response.json() as Promise<unknown> }
...
export function createApp() {
  ...
  app.get('/api/stocks/:ticker', async (request, response) => {
    ...
    if (!process.env.ALPHA_VANTAGE_API_KEY) return response.status(424).json(envelope([], 'Alpha Vantage', 'unavailable', 'Add ALPHA_VANTAGE_API_KEY to the deployment environment.'))
    ...
    if (used >= 20) return response.status(429).json(envelope([], 'Alpha Vantage', 'unavailable', 'The daily request budget has been reached. Cached tickers remain available; try again tomorrow.'))
