...
describe('budget calculations', () => {
  ...
  it('handles allocation validation and total-return values', () => { expect(validateAllocations([{ allocation: 50 }, { allocation: 50 }])).toBe(true); expect(validateAllocations([{ allocation: 60 }, { allocation: 50 }])).toBe(false); expect(investmentValue(10000, 100, 125)).toBe(12500); expect(investmentValue(10000, 0, 125)).toBeNull() })
})
