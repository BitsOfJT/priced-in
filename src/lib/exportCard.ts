...
export function downloadBudgetCard(result: ComparisonResult, includeCategories: boolean) {
  ...
  context.fillStyle = ivory; context.font = '500 27px ui-monospace, monospace'; context.fillText('PRICED IN  /  ESTIMATED SAME BASKET', 70, 80)
  context.font = '600 66px Georgia, serif'; context.fillText('The same life, priced in bitcoin.', 70, 157)
  const points = result.points.filter((point) => point.dollarIndex !== null)
  ...
  ;([{ key: 'dollarIndex', color: ivory }, { key: 'bitcoinIndex', color: copper }] as const).forEach(({ key, color }) => {
    context.strokeStyle = color; context.beginPath(); points.forEach((point, index) => { const value = point[key] ?? 0; index ? context.lineTo(x(index), y(value)) : context.moveTo(x(index), y(value)) }); context.stroke()
  })
